...
for(let i1 = 0; i1 < values.length; i1++)values[i1].style.fontSize = "35px";
for(let i2 = 0; i2 < labels.length; i2++){
    labels[i2].style.fontSize = "15px";
    labels[i2].style.fontWeight = "bold";
}
btn.disabled = true;
const options = {
    enableTime: true,
...
function addLeadingZero(number) {
    if (`${number}` < 10) return number = `${number}`.padStart(2, "0");
    else return `${number}`;
}
